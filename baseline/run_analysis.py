...
def analyse_repo(repo_url: str) -> dict:
    ...
    for action_name in ACTION_SEQUENCE:
        ...
        status = "OK" if "result" in info else info.get("error", info.get("warning", ""))
        print(f"  step  {observation.step_count:>2}  {action_name:<28}  reward_delta={reward.delta:+.4f}  {status}")
        final_info = info
