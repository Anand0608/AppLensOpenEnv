...
def _validate_repo_url(url: str) -> None:
    """Basic validation to allow only public HTTPS git URLs from known hosts."""
    parsed = urlparse(url)
    ...
    if not _GIT_URL_PATTERN.search(url):
        raise ValueError(
            f"URL does not match a supported Git hosting pattern "
            f"(github.com, gitlab.com, bitbucket.org, dev.azure.com, or *.git): {url!r}"
        )
