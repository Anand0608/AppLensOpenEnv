...
def run_agent(repo_url: str, model_path: str = DEFAULT_MODEL_PATH) -> dict:
    ...
    for step in range(MAX_STEPS):
        ...
        observation, reward, done, info = real_env.step(Action(action=action_name))
        total_reward = reward.total

        status = "✓" if "result" in info else info.get("warning", info.get("error", "?"))
        print(f"  step {step + 1:>2}  [{action_name:<28}]  delta={reward.delta:+.4f}  {status}")
